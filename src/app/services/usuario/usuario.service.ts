...
export class UsuarioService {
  ...
  login(usuario: Usuario, recordar: boolean = false) {
    ...
    return this.http.post(url, usuario).map((resp: any) => {
      ...
      swal('Error al logear', err.error.mensaje, 'error');
  ...
  crearUsuario(usuario: Usuario) {
    ...
    return this.http.post(url, usuario).map((resp: any) => {
      ...
      let mensaje = err.error.errors.message
      let mensajeCortado = mensaje.split(":");

      console.log(mensajeCortado)
      swal(err.error.mensaje, mensajeCortado[2], 'error');
  ...
  borrarUsuario(usuarioId: string) {
    let url = `${URL_SERVICIOS}/usuario/${usuarioId}/?token=${this.token}`;
